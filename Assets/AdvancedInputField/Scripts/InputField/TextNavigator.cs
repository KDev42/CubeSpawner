		...
		internal void KeepActionBarWithinBounds()
		{
			...
			if(normalizedLeftX < ACTION_BAR_MARGIN_X)
			{
				...
				if(Canvas.renderMode == RenderMode.WorldSpace)
				{
					...
					actionBarPosition.x += (ACTION_BAR_MARGIN_X - normalizedLeftX) * (canvasTransform.rect.width / ratioX / Canvas.scaleFactor);
				}
				else
				{
			...
			else if(normalizedRightX > 1 - ACTION_BAR_MARGIN_X)
			{
				...
				if(Canvas.renderMode == RenderMode.WorldSpace)
				{
					...
					Vector2 canvasWorldSize = new Vector2(Mathf.Abs(canvasCorners[3].x - canvasCorners[1].x), Mathf.Abs(canvasCorners[3].y - canvasCorners[1].y));
					float ratioX = canvasWorldSize.x / ((Canvas.worldCamera.orthographicSize * 2) / Screen.height * Screen.width);
					actionBarPosition.x += ((1 - ACTION_BAR_MARGIN_X) - normalizedRightX) * (canvasTransform.rect.width / ratioX / Canvas.scaleFactor);
				}
				else
				{
